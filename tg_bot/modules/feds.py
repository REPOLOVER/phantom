...
def new_fed(bot: Bot, update: Update):
    ...
    if user.id == int(OWNER_ID):
        ...
        if not fed:
            update.effective_message.reply_text("Can't federate! Please contact @AnonymousD3061 if the problem persists.")
            return
...
def join_fed(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        if not fed:
            message.reply_text("Failed to join federation! Please contact @AnonymousD3061 should this problem persists!")
            return
...
def fed_ban(bot: Bot, update: Update, args: List[str]):
    ...
    if not x:
        message.reply_text("Failed to ban from the federation! If this problem continues, contact @AnonymousD3061.")
        return
...
def set_frules(bot: Bot, update: Update, args: List[str]):
    ...
    if not x:
        update.effective_message.reply_text("Big F! There is an error while setting federation rules! If you wondered why please ask it in @AnonymousD3061 !")
        return
